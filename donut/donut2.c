#include <stdio.h>   // For printf, putchar
#include <string.h>  // For memset
#include <math.h>    // For sin, cos
#include <unistd.h>  // For usleep

int main() {
    float A = 0, B = 0;
    float i, j;
    int k;
    float z[1760];
    char b[1760];

    // Clear the console
    printf("\x1b[2J");
    
    // Infinite loop to animate the rotating donut
    for(;;) {
        // Clear the buffers
        memset(b, 32, 1760); // Clear character buffer
        memset(z, 0, 7040);  // Clear depth buffer

        // Create the donut shape
        for(j = 0; j < 6.28; j += 0.02) {
            for(i = 0; i < 6.28; i += 0.02) {
                float c = sin(i);
                float d = cos(j);
                float e = sin(A);
                float f = sin(j);
                float g = cos(A);
                float h = d + 2;
                float D = 1 / (c * h * e + f * g + 5);
                float l = cos(i);
                float m = cos(B);
                float n = sin(B);
                float t = c * h * g - f * e;

                // Calculate coordinates
                int x = 40 + 30 * D * (l * h * m - t * n);
                int y = 12 + 15 * D * (l * h * n + t * m);
                int o = x + 80 * y;
                int N = 8 * ((f * e - c * d * g) * m - c * d * e - f * g - l * d * n);

                // Only draw if within bounds
                if (22 > y && y > 0 && x > 0 && 80 > x && D > z[o]) {
                    z[o] = D;
                    b[o] = ".,-~:;=!*#$@"[N > 0 ? N : 0];
                }
            }
        }

        // Clear the screen and print the new frame
        printf("\x1b[H");

        // Set text color to green using ANSI escape code
        printf("\x1b[32m");  // Set text to green
        // printf("\x1b[33m");  // Set text to yellow
        // printf("\x1b[31m");  // Set text to red

        for(k = 0; k < 1761; k++) {
            putchar(k % 80 ? b[k] : 10);  // Print each character, add new line at the end of each row
        }

        // Reset the text color back to default
        printf("\x1b[0m");

        // Update angles for animation
        A += 0.36; // Adjusted to slow down rotation
        B += 0.36; // Adjusted to slow down rotation

        // Pause for a moment to slow down the animation
        usleep(20000); // Adjusted to slow down the animation
    }
    return 0;
}
