...
int main()
{
    try
    {
        ...
        // Save outputs
        double_vector_save_txt(box.potential, "potential.txt");
        double_vector_save_txt(box.geometry, "geometry.txt");


        // Add particles
        int particle_iterator = 0;
        for (const auto &entry : fs::directory_iterator("."))
        {
            fs::path filepath = entry.path();
            std::string filename = filepath.filename().string();
            ...
            if (filepath.extension() == ".json" &&
                filename.rfind("ParticleConfig_", 0) == 0)
            {
                std::ifstream file(filepath);
                if (!file.is_open())
                {
                ...
                if (!j.contains("particle") || !j["particle"].is_array() || j["particle"].empty())
                {
                    std::cerr << "Invalid or missing 'particle' in " << filename << "\n";
                    continue;
                }

                const auto &particle = j["particle"][0];
                std::string type = "Unknown";
                if (particle.contains("type") && particle["type"].is_string())
                {
                    type = particle["type"];
                }
                ...
                if (type == "Proton")
                {
                    ...
                    double x = particle.value("x", 0.0) * cm;
                    double y = particle.value("y", 0.0) * cm;
                    double z = particle.value("z", 0.0) * cm;

                    double energy = particle.value("Energy", 0.0) * kev_to_joule;

                    // Read and normalize direction vector
                    double vx = particle.value("x̂", 1.0);
                    double vy = particle.value("ŷ", 0.0);
                    double vz = particle.value("ẑ", 0.0);

                    std::tie(vx, vy, vz) = normalize_vector(vx, vy, vz);

                    // Compute relativistic gamma and scale velocities
                    double gamma = 1 + energy / (m * c * c);
                    ...
                    double max_dim = max_AbsoluteValue_double_vector({lx, ly, lz})*cm;
                    double t_max = 50 * max_dim / v;
                    double t_res = 20000;
                    double dt = t_max / t_res;

                    Particle p = Particle::__init__{.x = x, .y = y, .z = z, .vx = vx, .vy = vy, .vz = vz, .q = q, .m = m}.__build__();
                    propagator(p, box, t_max, dt);

                    std::string filename = "particle_track_" + std::to_string(particle_iterator++) + ".txt";
                    save_xyz_to_txt(p.posx, p.posy, p.posz, filename);
                    particle_iterator += 1;
                }
                ...
                {
                    double m = particle.value("mass(in mH)", 0.0) * mH;
                    double q = particle.value("charge (in qe)", 0.0) * qe;
